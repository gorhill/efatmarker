...
}}};this.syncDOMMenu=function(){var u=b.querySelector("#efm-permalink input"),q=c.location.href,s=this.spansToToken();
if(u){u.value=s?q:""}u=b.getElementById("efm-twitter-button");if(u){var r=[];r.push("url="+String(encodeURIComponent(q)));
...
j.preventDefault()})}f=b.querySelector("#efm-permalink > input");if(f){f.addEventListener("click",function(j){this.select()
})}h.syncDOMHighlights();h.syncDOMMenu();if(h.hasSpans()){if(typeof jQuery==="function"){jQuery("body,html").animate({scrollTop:parseInt(jQuery(".efm-parent").offset().top,10)-50})
