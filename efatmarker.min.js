...
}}};this.syncDOMMenu=function(){var s=document.getElementById("efm-permalink"),o=window.location.href,q=this.spansToToken();
if(s){s.innerHTML=q?"Permalink with highlights:<br>"+o.replace(/^(.{3,15}).+?(.{3,15})$/,"$1<i>...</i>$2"):"Permalink: [no highlights]";
s.href=o}s=document.getElementById("efm-twitter-button");if(s){var p=[];p.push("url="+String(encodeURIComponent(o)));
p.push("text="+encodeURIComponent(document.querySelector("head title").innerHTML));
var r=document.querySelector('head link[rel="canonical"]');if(r){p.push("counturl="+String(encodeURIComponent(r.href)))
}s.href="https://twitter.com/share?"+p.join("&")}var m=document.querySelectorAll(".efm-need-highlight"),t=m.length,l=this.hasSpans();
while(t--){m[t].style.display=l?"":"none"}};this.unrender=function(){var m=this.textmap,l=m.length-1,o,p,n;
...
h.preventDefault()})}f.syncDOMHighlights();f.syncDOMMenu();if(f.hasSpans()){if(typeof jQuery==="function"){jQuery("body,html").animate({scrollTop:parseInt(jQuery(".efm-parent").offset().top,10)})
