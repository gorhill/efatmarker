...
**/
(function(){var a=function(){var d=document.querySelector(".efm-target");if(!d){throw"EFatMarker: no element with class 'efm-target' found."
}this.parentNode=d;var e=[],f=0,g=d.childNodes.length,h,k=0,j=[],c,i,b;for(;;){while(f<g){c=d.childNodes[f];
...
if(p){p.innerHTML=s?"Permalink with highlights:<br>"+m.replace(/^(.{3,15}).+?(.{3,15})$/,"$1<i>...</i>$2"):"Permalink: [no highlights]";
p.href=m}p=document.getElementById("efm-twitter-button");if(p){var q=[];q.push("url="+String(encodeURIComponent(m)));
